...
(2026-06-11) [Git] After a squash-merge, the source branch's original commits look un-merged by SHA ancestry (git log base..branch still lists them), even though their content IS in the base. Don't trust commit count to judge what's merged — use a CONTENT diff: `git diff origin/poimen-main..branch --stat`. Empty = fully landed. ALSO: never keep committing on a squash-merged branch and re-PR it — the old commits conflict with the squashed result (got a 405 merge-conflict this session). Reset the branch onto poimen-main first (`git reset --hard origin/poimen-main`), re-apply changes, force-push.

(2026-06-11) [Tooling] In the remote (web) container, branches CANNOT be deleted from the CLI — the git proxy rejects ref deletions (`git push --delete` and `git push origin :branch` both hang up with "remote end hung up"), and the GitHub MCP exposes no delete-branch tool. Delete merged branches via the GitHub web UI ("Delete branch" button on the PR page).

(2026-06-11) [Deployment] Vercel redeploy is NOT possible from the remote container as configured: the Vercel CLI is installed but there are no credentials (no VERCEL_TOKEN, no .vercel link) AND network egress to vercel.com is blocked by the network policy (login returns "Host not in allowlist"). To deploy from here you'd need BOTH a VERCEL_TOKEN env var and vercel.com/api.vercel.com on the network allowlist. Otherwise run the deploy locally.

(2026-06-11) [Deployment] Vercel alias DOES NOT auto-update on new production deploys. Every `vercel . --prod` creates a new deployment URL; poimen-app.vercel.app stays frozen on the old one until explicitly moved. After every deploy, run: `vercel alias set <new-deployment-url> poimen-app.vercel.app --scope bishoy-saleeb-s-projects`. Without this, visitors to poimen-app.vercel.app see a stale build indefinitely. Full redeploy sequence: (1) `npx expo export --platform web` in poimen/, (2) `vercel . --yes --scope bishoy-saleeb-s-projects --prod` from poimen/, (3) `vercel alias set <new-url> poimen-app.vercel.app --scope bishoy-saleeb-s-projects`.
