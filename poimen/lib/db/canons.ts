...
export async function upsertCanonCompletion(canonId: string, userId: string, completedOn: string): Promise<{ error: string | null }> {
  const { error } = await supabase.from('canon_completions').upsert(
